...
{
    //~ committed default heap 2D texture with DSV
    //  it is a reversed Z with  8-bit stencil reserved
    //  non-shader-visible DSV heap, TODO: Add SRV later
    ...
    public:
        ...
        [[nodiscard]] bool initialize  (const hardware::device& dev,
                                        std::uint32_t width,
                                        std::uint32_t height);
                      void deinitialize() noexcept;

        //~ release current resource and recreate at new size
        // ensure gpu is idle before that its a must
        [[nodiscard]] bool resize(const hardware::device& dev,
                                  std::uint32_t width,
                                  std::uint32_t height);
    ...
    private:
        bool create_resource(const hardware::device& dev);
