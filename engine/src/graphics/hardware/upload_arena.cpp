...
namespace cots::graphics::hardware
{
    ...
    bool upload_arena::initialize(device& dev)
    {
        device_ = &dev;
        ...
        if (!d3d)
        {
            spdlog::error("[upload-arena] no device");
            return false;
        }
        ...
        {
            spdlog::error("[upload-arena] CreateCommandAllocator failed");
            return false;
        }
        ...
        {
            spdlog::error("[upload-arena] CreateCommandList failed");
            return false;
        }
        ...
        if (FAILED(d3d->CreateFence(0, D3D12_FENCE_FLAG_NONE,
                                    IID_PPV_ARGS(&fence_))))
        {
            spdlog::error("[upload-arena] CreateFence failed");
            return false;
        }
        ...
        batch_open_ = false;
        spdlog::info("[upload-arena] ready");
        return true;
    }

    void upload_arena::deinitialize()
    {
        free_pool_ .clear();
        in_use_    .clear();
        in_flight_ .clear();
    ...
    bool upload_arena::begin_batch()
    {
        if (batch_open_)
        {
            spdlog::warn("[upload-arena] begin_batch called with batch already open");
            return false;
        }
        if (!device_ || !cmd_alloc_ || !cmd_list_)
        {
            spdlog::error("[upload-arena] begin_batch before init");
            return false;
        }

        //~ reclaim everything the gpu is done with so the free pool is
        // full when add_* starts hitting it
        recycle_completed();

        if (FAILED(cmd_alloc_->Reset()))
        {
            spdlog::error("[upload-arena] cmd allocator reset failed");
            return false;
        }
        if (FAILED(cmd_list_->Reset(cmd_alloc_.Get(), nullptr)))
        {
            spdlog::error("[upload-arena] cmd list reset failed");
            return false;
        }
    ...
    bool upload_arena::allocate_staging(const std::uint64_t size, staging& out)
    {
        ...
        if (!allocator)
        {
            spdlog::error("[upload-arena] no allocator");
            return false;
        }
        ...
        {
            spdlog::error("[upload-arena] staging CreateResource failed for {} bytes", size);
            return false;
        }
        ...
        {
            spdlog::error("[upload-arena] staging Map failed");
            alloc->Release();
            return false;
        }
    ...
    upload_arena::fence_value upload_arena::submit()
    {
        if (!batch_open_)
        {
            spdlog::warn("[upload-arena] submit called without begin_batch");
            return 0u;
        }
        ...
        {
            spdlog::error("[upload-arena] cmd list Close failed");
            batch_open_ = false;
            return 0u;
        }
        ...
        {
            spdlog::error("[upload-arena] queue Signal failed");
            batch_open_ = false;
            return 0u;
        }
    ...
    bool upload_arena::wait(const fence_value v) const
    {
        ...
        if (!ev)
        {
            spdlog::error("[upload-arena] CreateEvent failed in wait");
            return false;
        }
        if (FAILED(fence_->SetEventOnCompletion(v, ev)))
        {
            spdlog::error("[upload-arena] SetEventOnCompletion failed");
            ::CloseHandle(ev);
            return false;
        }
    ...
    void upload_arena::cancel_batch()
    {
        ...
        if (cmd_list_)
        {
            if (FAILED(cmd_list_->Close()))
            {
                spdlog::warn("[upload-arena] cancel_batch Close failed");
            }
        }
        batch_open_ = false;
    }
