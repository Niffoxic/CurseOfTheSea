...
#include "engine/graphics/hardware/depth_target.h"
...
namespace cots::graphics::hardware
{
    ...
    bool depth_target::initialize(const device& dev,
    ...
    bool depth_target::resize(const device& dev,
                              const std::uint32_t width,
                              const std::uint32_t height)
    {
        if (!dsv_heap_)
    ...
    bool depth_target::create_resource(const device& dev)
    {
        auto* d3d = dev.d3d12_device();
        ...
        //~ reversed-Z: optimized clear is depth 0.0, stencil 0
        ...
        //~ DSV: depth-stencil view of the texture
