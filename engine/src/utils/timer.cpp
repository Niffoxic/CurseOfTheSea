...
void cots::timer::reset() noexcept
{
...
void cots::timer::pause() noexcept
{
    paused_ = true;
}

void cots::timer::resume() noexcept
{
...
void cots::timer::set_target_frame_ps(int target_frame_rate) noexcept
{
...
float cots::timer::elapsed_time_ms() const noexcept
{
    const auto end = clock::now();
    const auto e   = std::chrono::duration_cast<ns>(end - start_time_);
...
std::string cots::timer::current_date() noexcept
{
...
bool cots::timer::paused() const noexcept
{
    return paused_;
}
