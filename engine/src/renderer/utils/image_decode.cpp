...
//~ this is the one and only tu that pulls in the stb implementation
...
namespace trishul::render::utils
{
    bool decode_image_file(const std::string_view path, decoded_image& out)
    {
        ...
        //~ forcing rgba8 out no matter what the source channel count is
        stbi_uc* pixels = stbi_load(
            spath.c_str(), &w, &h, &channels_in_file, STBI_rgb_alpha);

        if (!pixels)
        {
            LOG_ERROR("[image] decode failed for '{}' reason '{}'",
        ...
        LOG_INFO("[image] decoded '{}' {}x{}", spath, out.width, out.height);
        return true;
    }
    ...
    {
        out = decoded_image{};
        //~ clamping the size up to at least one so we never hand back an empty
        //~ image even if the caller passes zeros
        ...
        for (std::uint32_t y = 0; y < out.height; ++y)
        {
            ...
            {
                //~ flipping colour every cell along both axes the classic board
